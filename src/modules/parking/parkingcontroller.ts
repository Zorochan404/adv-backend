...
export const getParking = asyncHandler(async (req: Request, res: Response) => {
    try {
        ...
        return res.status(200).json(new ApiResponse(200, parking, "Parking fetched successfully"));
    } catch (error) {
        console.log(error);
...
export const getParkingByFilter = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (name) {
            conditions.push(like(sql`lower(${parkingTable.name})`, `%${name.toLowerCase()}%`));
        }
        
        if (city) {
            conditions.push(like(sql`lower(${parkingTable.city})`, `%${city.toLowerCase()}%`));
        }
        
        if (locality) {
            conditions.push(like(sql`lower(${parkingTable.locality})`, `%${locality.toLowerCase()}%`));
        }
        
        if (country) {
            conditions.push(like(sql`lower(${parkingTable.country})`, `%${country.toLowerCase()}%`));
        }
        
        // If no filters provided, return all parking
        if (conditions.length === 0) {
            const parking = await db.select().from(parkingTable);
            return res.status(200).json(new ApiResponse(200, parking, "All parking fetched successfully"));
        }
        ...
        if (parking.length === 0) {
            return res.status(200).json(new ApiResponse(200, [], "No parking found with the specified filters"));
        }
        
        return res.status(200).json(new ApiResponse(200, parking, "Filtered parking fetched successfully"));
    } catch (error) {
        console.log(error);
...
export const getNearByParking = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
        ...
        const parking = await db
            ...
            if(parking.length === 0){
                return res.status(200).json(new ApiResponse(200, [], "No parking found"));
            }

        return res.status(200).json(new ApiResponse(200, parking, "Nearby parking fetched successfully"));
    } catch (error) {
        console.log(error);
        if (error instanceof ApiError) {
            throw error;
...
export const getParkingById = asyncHandler(async (req: Request, res: Response) => {
    try {
        ...
        const parking = await db.select().from(parkingTable).where(eq(parkingTable.id, parseInt(id)));
        ...
        return res.status(200).json(new ApiResponse(200, result, "Parking with cars fetched successfully"));
    } catch (error) {
        console.log(error);
        if (error instanceof ApiError) {
            throw error;
...
export const createParking = asyncHandler(async (req: Request & { user?: { role?: string } }, res: Response) => {
    try {
        if (
            req.user &&
            (
            ...
            const parking = await db.insert(parkingTable).values(req.body).returning();
            
            return res.status(200).json(new ApiResponse(200, parking, "Parking added successfully"));
        } else {
...
export const updateParking = asyncHandler(async (req: Request & { user?: { role?: string } }, res: Response) => {
    try {
        if (
            req.user &&
            (
            ...
            const { id } = req.params;
            const parking = await db.update(parkingTable).set(req.body).where(eq(parkingTable.id, parseInt(id))).returning();

            return res.status(200).json(new ApiResponse(200, parking, "Parking updated successfully"));
        } else {
...
export const deleteParking = asyncHandler(async (req: Request & { user?: { role?: string } }, res: Response) => {
    try {
        if (
            req.user &&
            (
            ...
            const { id } = req.params;
            const parking = await db.delete(parkingTable).where(eq(parkingTable.id, parseInt(id)));

            return res.status(200).json(new ApiResponse(200, parking, "Parking deleted successfully"));
        } else {
...
export const getParkingByIDadmin = asyncHandler(async (req: Request & { user?: { role?: string } }, res: Response) => {
    try {
        if (req.user && (req.user.role === "admin" || req.user.role === "parkingincharge")) {
            const { id } = req.params;
            ...
            return res.status(200).json(new ApiResponse(200, result, "Parking details with incharge and cars fetched successfully"));
        } else {
